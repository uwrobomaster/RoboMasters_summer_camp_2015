...
gimbal_data  drone_gimbal_data={1,1,1,1};//定义角度信息 为方格图像检测输入信息

pthread_mutex_t mymutex=PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t myimshow=PTHREAD_MUTEX_INITIALIZER;
...
//    resize(srcImage, srcImage, Size(150, 100));
   ...
   if (contours.empty() || hierarchy.empty()) return;
    ...
    {
      pthread_mutex_lock(&myimshow);
      imshow("Ball_showcircle", showcircle);
      pthread_mutex_unlock(&myimshow);
    }
...
void  Imag_Dect_Square_Function(int color,Mat srcimage){
        ...
        srcImage=srcimage.clone();
        resize(srcImage, srcImage, Size(150, 100));
        if(drone_gimbal_data.height<0.5) return;
//        resize(srcImage, srcImage, Size(150, 100));
 ...
 */
 void Imag_Dect_Function( Mat srcimage){//修改为inline 函数？？？？？？
       Mat srcImage, midImage, dstImage;
       srcImage=srcimage.clone();
       cout<<"line dect open"<<endl;
        resize(srcImage, srcImage, Size(150, 100));
    // double time = (double)getCPUTickCount();
    // for (int i=0;;i++){
        //double time = (double)getCPUTickCount();
        // capture >> srcImage;//获取图像
        cout<<"line dect open22222222"<<endl;
        if (srcImage.empty()) return;
        if(drone_gimbal_data.height<0.5) return;
        cout<<"drone_gimbal_data.height: "<<drone_gimbal_data.height<<endl;
...
//       // imshow("thined image", dstImage);
       ...
       // cout << "斜率： " << returntheta << " 截距： " << returnrho << endl;
        ...
        if(lines.size()>200){
            ...
            drone_image_data.bestrho=returnrho+tan(mye[2]*CV_PI/180)*drone_gimbal_data.height*deltaCam;
