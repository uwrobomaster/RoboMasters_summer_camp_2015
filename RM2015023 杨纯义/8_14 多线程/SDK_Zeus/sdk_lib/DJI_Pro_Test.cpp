...
/////////////////////////////////global variables///////////////////////////////////////
float uav_height;
float camera_state[3];//pitch, yaw, roll
VO_OUTPUT vo_data;
float drone_attitude[3];
//data buffer from ground
int pbuf_t[4];//point1.x / point1.y / point2.x / point2.y
char rbuf[128];
int rbuf_len;
bool data_recv;
uav_data_to_ground drone_data;
...
int16_t sdk_pure_transfer_handler(uint8_t cmd_id,uint8_t* pbuf,uint16_t len,req_id_t req_id);
...
cmd_handler_table_t cmd_handler_tab[] =
{
    ...
    {0x02,sdk_pure_transfer_handler			},
	{ERR_INDEX,NULL							}
};
...
//get data from ground(two points) : point1.y / point2.x / point2.y
int16_t sdk_pure_transfer_handler_two_points(uint8_t cmd_id,uint8_t* pbuf,uint16_t len,req_id_t req_id)
{
    //pthread_mutex_lock(&status_lock_s);
        data_recv = false;
        rbuf_len +=  len;
        printf("get ground data bytes : %d \n" , len);
        ...
        else
        {
            pbuf_t[2] = temp[0] * 640;
            pbuf_t[3] = temp[1] * 360;
            printf("recieve data from ground\n");
            for(int i = 0; i < rbuf_len / sizeof(float); i++)
                {
                    printf("%d\n",pbuf_t[i]);
                }
            data_recv = true;
...
//get data from ground(single point) : point.x/point.y(in fact, ratio in x,ratio in y)
int16_t sdk_pure_transfer_handler(uint8_t cmd_id,uint8_t* pbuf,uint16_t len,req_id_t req_id)//data[0] is ID (tracking ID is 0, uav ID is 1)
{
    data_recv = true;
    rbuf_len = len;
    printf("get ground data bytes : %d \n" , len);
...
int16_t sdk_std_msgs_handler(unsigned char cmd_id,unsigned char* pbuf,unsigned short len,req_id_t req_id)
{
	...
	_recv_std_msgs( *msg_enable_flag, ENABLE_MSG_DEVICE	, recv_sdk_std_msgs.ctrl_device			, pbuf, data_len);
      ...
      if( (*msg_enable_flag & ENABLE_MSG_POS) )//UAV pos
        {
            pthread_mutex_lock(&status_lock_s);
            uav_height = recv_sdk_std_msgs.pos.height;
            vo_data.x = recv_sdk_std_msgs.pos.lati;
            vo_data.y = recv_sdk_std_msgs.pos.longti;
            vo_data.z = recv_sdk_std_msgs.pos.alti;
            pthread_mutex_unlock(&status_lock_s);
        }
      if( (*msg_enable_flag & ENABLE_MSG_V) )//UAV v
        {
            pthread_mutex_lock(&status_lock_s);
            vo_data.vx = recv_sdk_std_msgs.v.x;
            vo_data.vy = recv_sdk_std_msgs.v.y;
            vo_data.vz = recv_sdk_std_msgs.v.z;

            float vx_body_level = vo_data.vx * cos(yaw) + vo_data.vy * sin(yaw);
            float vy_body_level = vo_data.vx * (-sin(yaw)) + vo_data.vy * cos(yaw);
            float vz_body_level = vo_data.vz;

            pthread_mutex_unlock(&status_lock_s);
        }
      if( (*msg_enable_flag & ENABLE_MSG_GIMBAL) )
        {
            pthread_mutex_lock(&status_lock_s);
            camera_state[2] = recv_sdk_std_msgs.gimbal.x;
            camera_state[0] = recv_sdk_std_msgs.gimbal.y;
            camera_state[1] = recv_sdk_std_msgs.gimbal.z;
            pthread_mutex_unlock(&status_lock_s);
        }
      if( (*msg_enable_flag & ENABLE_MSG_Q) )
        {
            pthread_mutex_lock(&status_lock_s);
            float q[4];
            q[0] = recv_sdk_std_msgs.q.q0;
            q[1] = recv_sdk_std_msgs.q.q1;
            q[2] = recv_sdk_std_msgs.q.q2;
            q[3] = recv_sdk_std_msgs.q.q3;

            q2e(q, drone_attitude);
            yaw = drone_attitude[0]; //yaw
            //printf("drone-yaw-pitch-roll: %f, %f, %f\n", drone_attitude[0]*180/3.1415926, drone_attitude[1]*180/3.1415926, drone_attitude[2]*180/3.1415926);

            //drone_attitude[1] pitch
            //drone_attitude[2] roll
            pthread_mutex_unlock(&status_lock_s);
        }
...
void spin_callback(void)
{
	...
	count++;
    /******PID Parameter*******/
    myq[0]=(float)recv_sdk_std_msgs.q.q0;
    myq[1]=(float)recv_sdk_std_msgs.q.q1;
    myq[2]=(float)recv_sdk_std_msgs.q.q2;
    myq[3]=(float)recv_sdk_std_msgs.q.q3;
    q2e(myq,mye);
    drone_gimbal_data.yaw=mye[0];
    drone_gimbal_data.pitch=mye[1];
    drone_gimbal_data.roll=mye[2];
    drone_gimbal_data.height=(float)recv_sdk_std_msgs.pos.height;
    /**************************/
...
//       printf("SpeedZ ********************************************************* %f \n",recv_sdk_std_msgs.v.z);
//       printf("Height********************************************************** %f \n",recv_sdk_std_msgs.pos.height);
//       printf("[STD_MSGS] rc ************************************%d\n",recv_sdk_std_msgs.rc.roll);
