...
// Mat srcImage, midImage, dstImage,src_Image;
 Mat src_Image;
int para = 0;
        ...
        if (!contours.empty() && !hierarchy.empty())
                {
                    ...
                    for (int idx = 0; idx >= 0; idx = hierarchy[idx][0])
                    {
                       ...
                       if (arealength <40){
                           drone_image_data.blackDetected=false;
                           drone_image_data.squareDetected=false;
                           continue;
                       }
                       cout<<"arealength: "<<arealength<<endl;
                        ...
                        if(color==0) drone_image_data.blackDetected=true;
                        if(color==1) drone_image_data.squareDetected=true;
 ...
 */
 void Imag_Dect_Function( Mat srcimage){//修改为inline 函数？？？？？？
    ...
    // for (int i=0;;i++){
        //double time = (double)getCPUTickCount();
        // capture >> srcImage;//获取图像
       if (srcImage.empty()) return;
        if(Act_Hei<0.5) return;
       cout<<"Act_Hei: "<<Act_Hei<<endl;
//       imshow("src123",srcImage);
        //【1】载入原始图和Mat变量定义
//        resize(srcImage, srcImage, Size(150, 100));
...
//    waitKey(0);
   ...
   // Image.m_draw=true;
    ...
    // Image.set_CamSize(640,480);//修改   width,  height
     ...
     Image.capture>>src_Image;//讀取是否需要放在另外一個綫程
     Start_Dect_Square_Thread();
...
//      statues=detect_ball_square_ready;
    for (int i = 0;; )
    {
        ...
        Image.capture>>src_Image;//讀取是否需要放在另外一個綫程
        resize(src_Image, src_Image, Size(150, 100));
        waitKey(10);
        ...
        {   case detect_line_ready://线检测初始设置
            ...
            case detect_9_square://九宫格算法检测
            {
                // Image.set_CamParameter(-1,-1,-1);//修改 exposure, gain, brightness
                // Image.set_CamSize(640,480);//修改   width,  height
                Imag_Dect_Square_Function(1,src_Image);
            }
            break;
            ...
            {
             Imag_Dect_Square_Function(0,src_Image);
            }
            ;break;
            case detect_line_square_ready://line  square
            {
            ...
            ;break;
            case detect_ball_square_ready://
            {
            statues=detect_ball_square;
            }
            ;break;
            case detect_ball_square://
            {
            Image_Dect_Ball(src_Image);
            }
            ;break;
...
static void * Square_Dect(void * arg){
...
int Start_ImagDect_Thread(void)
{
    int ret;
    pthread_t A_ARR;
    ret = pthread_create(&A_ARR, 0,Imag_Dect,NULL);
    if(ret != 0)
    {
        return -1;
    }
 ...
 * @Output:
*/
int Start_Dect_Square_Thread(void)
{
    int ret;
    pthread_t A_ARR;
    ret = pthread_create(&A_ARR, 0,Square_Dect,NULL);
    if(ret != 0)
    {
        return -1;
    }
