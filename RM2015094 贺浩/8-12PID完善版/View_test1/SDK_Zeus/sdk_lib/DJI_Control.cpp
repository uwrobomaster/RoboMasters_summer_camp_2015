...
/**********SPIN**************/
extern Yaw_Spin_PID Yaw_Spin;
extern float mye[3];  //mye[0]=yaw
int flag_YawSpin;
float Exp_Yaw;
/****************************/
float SpeedZ_out,Rate_YAW_out,SpeedY_out,SpeedX_out,Yaw_Spin_out;
/*****************************/
static void * Control_Thread_Function(void * arg)
 {
 	...
 	{		
               ...
               /*******Rho_Adj************/
                ...
                /********X_Offset_Adj********/
                SpeedX_out=PID_X_Offset_Adj(X_Offset_Actual);
                if(count%50==0)
                {
                    printf("SpeedX_out %f \n",SpeedX_out);
                }
                ...
                {
                    drone_ctrl_data.x=SpeedX_out;
                }
                /*********Yaw_Spin_Adj******/
                if(flag_YawSpin)
                {
                    Yaw_Spin_out=PID_Yaw_Spin_Adj(Exp_Yaw,mye[0]);
                    if(count%50==0)
                    {
                        printf("Yaw_Spin_out %f \n",Yaw_Spin_out);
                    }
                    if(Yaw_Spin_out>=20)
                    {
                        drone_ctrl_data.w=20;
                    ...
                    {
                        drone_ctrl_data.w=Yaw_Spin_out;
                    }
                }
...
void Theta_PIDinit()
{
    ...
    Theta.err_last=0.0;
//    Theta.err_next=0.0;
//    Theta.Kp=1;    //Optimal solution:Kp=1;Kd=2
//    Theta.Ki;
//    Theta.Kd=2;
}
...
void Rho_PIDinit()
{
    ...
    Rho.err_last=0.0;
//  Rho.err_next=0.0;
//  Rho.Kp=0.003;   //Optimal solution:Kp=0.004;Kd=0.025
//  Rho.Ki;
//  Rho.Kd=0.025;
}
...
void X_Offset_PIDinit()
{
    ...
    X_Offset.Kp=0.003;   //Optimal solution:Kp=0.004;Kd=0.025
    X_Offset.Ki;
    X_Offset.Kd=0.025;
...
{
    float Yaw_Spin_Increment;
    Yaw_Spin.err=exp_yaw-act_Yaw;
    Yaw_Spin_Increment=Yaw_Spin.Kp*Yaw_Spin.err+Yaw_Spin.Kd*(Yaw_Spin.err-Yaw_Spin.err_last);
    Yaw_Spin.err_last=Yaw_Spin.err;

    return Yaw_Spin_Increment;
}
